...
function convertValue(val: AssignTemplate, toType: ValType): AssignTemplate {
  ...
  if (toType === 'array') return Array.isArray(val) ? val : val !== undefined ? [val] : [];
  if (toType === 'object') {
...
/** 简单类型值的内联编辑器 */
function SimpleValueEditor({ value, onChange }: { value: AssignTemplate; onChange: (v: AssignTemplate) => void }) {
...
/** 递归的值编辑器 — 根据类型选择渲染方式 */
function TemplateValueEditor({ value, onChange }: { value: AssignTemplate; onChange: (v: AssignTemplate) => void }) {
  const [currentType, setCurrentType] = useState<ValType>(() => guessType(value));
  ...
  const handleTypeChange = useCallback((newType: ValType) => {
    setCurrentType(newType);
    onChange(convertValue(value, newType));
  }, [value, onChange]);

  // 所有类型按钮 — 始终显示全部 5 种选项
  const allTypes: ValType[] = ['string', 'number', 'boolean', 'array', 'object'];
      ...
      <div>
        <div className={styles.typeSelector}>
          {allTypes.map((t) => (
            ...
            >
              {t === 'string' ? '文本' : t === 'number' ? '数字' : t === 'boolean' ? '布尔' : t === 'array' ? '数组' : '对象'}
            </button>
          ))}
        </div>
...
/** 数组编辑器 */
function ArrayEditor({ value, onChange }: { value: AssignTemplate[]; onChange: (v: AssignTemplate[]) => void }) {
    ...
    <div>
      {value.map((item, i) => (
        <div key={i} className={styles.row}>
          ...
          <div style={{ flex: 1 }}>
            <TemplateValueEditor value={item} onChange={(v) => updateItem(i, v)} />
          </div>
          <button className={styles.deleteBtn} onClick={() => removeItem(i)} title="删除">×</button>
        </div>
      ))}
      <button className={styles.addBtn} onClick={addItem}>+ 添加元素</button>
    </div>
  );
}

/** 对象编辑器 */
function ObjectEditor({ value, onChange }: { value: Record<string, AssignTemplate>; onChange: (v: Record<string, AssignTemplate>) => void }) {
  ...
  const updateKey = (oldKey: string, newKey: string, newVal?: AssignTemplate) => {
    ...
    <div>
      {Object.entries(value).map(([key, val]) => (
        <div key={getStableId(key)} className={styles.row}>
          ...
          />
          <div style={{ flex: 1 }}>
            <TemplateValueEditor value={val} onChange={(v) => updateVal(key, v)} />
          </div>
          <button className={styles.deleteBtn} onClick={() => deleteKey(key)} title="删除">×</button>
        </div>
      ))}
      <button className={styles.addBtn} onClick={addField}>+ 添加字段</button>
    </div>
  );
}

export default function TemplateEditor({ value, onChange, onClose }: TemplateEditorProps) {
  return (
