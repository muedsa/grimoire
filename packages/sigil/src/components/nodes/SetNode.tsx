...
import { SetNode as SetNodeType } from "@/store/flow";
import { templateSummary } from "@/utils/templateSummary";
import { useNodeState } from "@/hook/useNodeState";
import DebugBadge from "./DebugBadge";
import styles from "./SetNode.module.css";

function SetNode({ data, selected, id }: NodeProps<SetNodeType>) {
    ...
    >
      ...
      <div className={styles.body}>
        <span className={styles.preview}>
          {config.variable || "variable"} = {templateSummary(config.value, 20)}
        </span>
      </div>
