...
import { ForeachNode } from "@/store/flow";
import { useNodeState } from "@/hook/useNodeState";
import DebugBadge from "./DebugBadge";
import { List } from "lucide-react";
import styles from "./ForeachGroup.module.css";

/**
 * Foreach 容器 — 作为 group node 渲染在子节点下方。
 * 虚线边框 + header 标签，直观标识同一 body 内的节点。
 * 调试态通过 useNodeState 显示执行高亮样式和循环迭代徽章。
 */
function ForeachGroup({ data, selected, id }: NodeProps<ForeachNode>) {
    ...
    >
      ...
      <div className={styles.header}>
        ...
        <span className={styles.label}>
          {config.collection && config.item
            ? config.index
              ? `Foreach ((${config.item}, ${config.index}) in ${config.collection})`
              : `Foreach (${config.item} in ${config.collection})`
            : "Foreach"}
        </span>
      </div>
...
export default memo(ForeachGroup);
